...
BEGIN
  ...
  IF _estimated_cost IS NULL OR _estimated_cost < 0 THEN _estimated_cost := 0; END IF;

  SELECT credits INTO _balance FROM public.profiles WHERE id = _uid FOR UPDATE;
  IF _balance IS NULL THEN RAISE EXCEPTION 'profile missing' USING ERRCODE = 'P0002'; END IF;
  IF _balance < _estimated_cost THEN RAISE EXCEPTION 'insufficient_credits' USING ERRCODE = 'P0001'; END IF;
